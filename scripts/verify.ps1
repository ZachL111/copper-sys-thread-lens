...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper-sys-thread-detail.ps1

Rscript tests/test_domain_review.R

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper-sys-thread-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper-sys-thread-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-copper_sys_thread_lens-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
